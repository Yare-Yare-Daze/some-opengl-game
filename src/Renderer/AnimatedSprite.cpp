...
namespace Renderer
{
	...
	void AnimatedSprite::setState(const std::string newState)
	{
		...
		// Проверка на существующий итератор и сбрасывание счетчиков
	...
	void AnimatedSprite::update(const uint64_t delta)
	{
		if (m_pCurrentAnimationDuration != m_statesMap.end())
		{
			...
			// Сравниваем текущее время анимации с временем анимации заданым в параметре
			while (m_currentAnimationTime >= m_pCurrentAnimationDuration->second[m_currentFrame].second)
			{
				// текущее время   дельта   результат        результат  время анимации  переходит в другой кадр анимации
				// 0.75          + 0.5    = 1.25        =>   1.25     - 1.00          = 0.25
				...
				{
					m_currentFrame = 0; // Можно сказать здесь проиходит зацикливание анимации
				}
			}
		}
	}
	...
	{
		// Обновляем текстуру не каждый кадр, а только когда переменная "граязная"
		if (m_dirty)
		{
			...
			glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(textureCoords), &textureCoords); // Обновление данных в буфере текстуры
