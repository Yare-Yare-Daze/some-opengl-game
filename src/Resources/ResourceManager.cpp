...
{
	size_t found = executablePath.find_last_of("/\\"); // Ищем послдений в строке '/' или '\' 
...
std::shared_ptr<Renderer::Texture2D> ResourceManager::loadTexture(const std::string& textureName, const std::string& texturePath)
{
	...
	std::shared_ptr<Renderer::Texture2D> newTexture = m_texturesMap.emplace
	(
		textureName, 
	...
	const std::string& subTextureName)
{
	...
	std::shared_ptr<Renderer::Sprite> newSprite = m_spritesMap.emplace(
		spriteName,
